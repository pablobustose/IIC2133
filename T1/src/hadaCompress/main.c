...
int main(int argc, char** argv)
{
  ...
  Nodo* raiz = crear_nodo(0, 0, ancho, alto);
  //Se crea el arbol recursivamente
  crear_arbol(raiz, img);
  llenar_promedios(raiz);
  calcular_suma2(raiz);
  llenar_desvestas(raiz);
  ...
  else if (string_equals(MODE, "compress"))
  {
    ...
    alpha = minimo_alpha(raiz, 64, 0, 128, h);
    filtro_alpha(raiz, alpha, img);
  }
  ...
  // Pintamos un cuadrado de ejemplo (Importante: Esto es solo para probar que la libreria funciona, puede ser eliminado)
  // img_square_paint(
